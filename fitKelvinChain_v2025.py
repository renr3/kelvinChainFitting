...
class kelvinChainModel ():
    ...
    def __init__(self, ageingComplianceSeries:list, complianceVariation:list):
        """
        Class constructor.
        ...
        Units:
            times must be in days, and start from the first day of age (on the same origin as the age)
            compliances must be in 1/MPa
        """
        ...
        self.complianceVariation=complianceVariation

        #Additional paramters that may be computed along the various Kelvin Chain fitting strategies available
        self.simplifiedPowerLawModel=None
        self.doublePowerLawModel=None
        self.modifiedDoublePowerLawModel=None
        self.dirichletSeriesModel=None

    #Smoothing curves method
    def fitDoublePowerLaw(self, typeOfDPL:str='classical', graphVisualization:bool=False, mrkSz:int=5, plotSlicingFactor:int=1, axisObjectToPlot:object=None, arbitrate_n_m=False, plotOriginalSeries=False):
        """
        ...
        arbitrate_n_m (list of 2 values): default to False, but if informed the fitting algorithm will adopt the values of [n,m] as fixed
        """
        ...
        # try:
        emptyList=[]
        [emptyList.extend([self.complianceAges[index] for element in complianceSeries[0]]) for index, complianceSeries in enumerate(self.complianceSeries)]
        t_line=np.array(emptyList)
        emptyList=[]
        [emptyList.extend(complianceSeries[0]) for complianceSeries in self.complianceSeries]
        t=np.array(emptyList)
        emptyList=[]
        [emptyList.extend(complianceSeries[1]) for complianceSeries in self.complianceSeries]
        ydata=np.array(emptyList)
        emptyList=[]
        [emptyList.extend(complianceSeries) for complianceSeries in self.complianceVariation]
        ydata_var=np.array(emptyList)
        # ydata=np.array([complianceSeries[1] for complianceSeries in self.complianceSeries])
        
        #Store the conversion factors
        conversionFactor_compliance=ydata.mean()
        ...
        ydata_var_norm=ydata_var.flatten()

        #Fit the correct type of DPL
        if typeOfDPL == 'classical':
            #Perform the fitting with initial guess given by Bazant and Osman
            if arbitrate_n_m is False:
                popt, pcov = curve_fit(self.DPLfunction, (t_norm, t_line_norm), ydata_norm, p0=[0.5, 0.5, 1/8, 1/3],sigma=ydata_norm)
            ...
            elif isinstance(arbitrate_n_m, list) and len(arbitrate_n_m) == 2 and all(isinstance(i, (int, float)) for i in arbitrate_n_m):
                from functools import partial
                DPL_fixed = partial(self.DPLfunction, m=arbitrate_n_m[1], n=arbitrate_n_m[0])
                # Now fit the data with only the free parameters E_0 and ϕ_1
                popt, pcov = curve_fit(DPL_fixed, (t_norm, t_line_norm), ydata_norm, p0=[0.5, 0.5],sigma=ydata_norm)
                #Store optimizaton results
                self.doublePowerLawModel={'model': 'DPL','E0':popt[0],'ϕ_1':popt[1],'m':arbitrate_n_m[1],'n':arbitrate_n_m[0],'conversionFactor_compliance': conversionFactor_compliance, 'pcov':pcov}
        elif typeOfDPL == 'modified':
            if arbitrate_n_m is False:
                popt, pcov = curve_fit(self.modifiedDPLfunction, (t_norm, t_line_norm), ydata_norm, p0=[0.5, 0.5, 1, 1, 1/8, 1/3],sigma=ydata_norm)
                #Store optimizaton results
                self.modifiedDoublePowerLawModel={'model': 'modifiedDPL','E0':popt[0],'ϕ_1':popt[1],'p':popt[2],'q':popt[3],'m':popt[4],'n':popt[5],'conversionFactor_compliance': conversionFactor_compliance, 'pcov':pcov}
            elif isinstance(arbitrate_n_m, list) and len(arbitrate_n_m) == 2 and all(isinstance(i, (int, float)) for i in arbitrate_n_m):
                from functools import partial
                modifiedDPL_fixed = partial(self.modifiedDPLfunction, m=arbitrate_n_m[1], n=arbitrate_n_m[0])
                # Now fit the data with only the free parameters E_0 and ϕ_1
                popt, pcov = curve_fit(modifiedDPL_fixed, (t_norm, t_line_norm), ydata_norm, p0=[0.5, 0.5, 1, 1],sigma=ydata_norm)
                #Store optimizaton results
                self.modifiedDoublePowerLawModel={'model': 'DPL','E0':popt[0],'ϕ_1':popt[1], 'p':popt[2],'q':popt[3], 'm':arbitrate_n_m[1],'n':arbitrate_n_m[0],'conversionFactor_compliance': conversionFactor_compliance, 'pcov':pcov}
        elif typeOfDPL == 'simplified':
            if arbitrate_n_m is False:
                popt, pcov = curve_fit(self.simplifiedDPLfunction, (t_norm, t_line_norm), np.log10(ydata_norm), p0=[0.5, 0.5, 0.5, 1/3, 1/8],maxfev = 100000)
                #Store optimizaton results
                self.simplifiedPowerLawModel={'model': 'sDPL','ϕ_1':popt[0],'n':popt[4], 'm':popt[3], 'p':popt[1], 'q':popt[2], 'conversionFactor_compliance': conversionFactor_compliance, 'pcov':pcov}
            elif isinstance(arbitrate_n_m, list) and len(arbitrate_n_m) == 2 and all(isinstance(i, (int, float)) for i in arbitrate_n_m):
                from functools import partial
                modifiedsimplifiedDPL_fixed = partial(self.simplifiedDPLfunction, m=arbitrate_n_m[1], n=arbitrate_n_m[0])
                # Now fit the data with only the free parameters E_0 and ϕ_1
                popt, pcov = curve_fit(modifiedsimplifiedDPL_fixed, (t_norm, t_line_norm), np.log10(ydata_norm), p0=[0.5, 0.5, 0.5])
                #Store optimizaton results
                self.simplifiedPowerLawModel={'model': 'sDPL','ϕ_1':popt[0], 'p':popt[1],'q':popt[2], 'm':arbitrate_n_m[1],'n':arbitrate_n_m[0],'conversionFactor_compliance': conversionFactor_compliance, 'pcov':pcov}


        #Show how good the optimization was
        if graphVisualization is True:
            color = iter(plt.cm.viridis(np.linspace(0, 1, len(self.complianceSeries))))
                ...
                if typeOfDPL == 'classical':
                    ...
                    if axisObjectToPlot is None:
                        if arbitrate_n_m is False:
                            ...
                            plt.plot(complianceSeries[0][::plotSlicingFactor]-self.complianceAges[enum],(np.array([self.modifiedDPLfunction((t,self.complianceAges[enum]),popt[0],popt[1],popt[2],popt[3],arbitrate_n_m[1],arbitrate_n_m[0]) for t in complianceSeries[0][::plotSlicingFactor]]))*conversionFactor_compliance*(1e6), c=c, marker='x',markersize=mrkSz,linestyle='--', label="DPL-"+"{:.1f}".format(self.complianceAges[enum])+"d",zorder=99)
                    else: 
                        axisObjectToPlot.plot(complianceSeries[0][::plotSlicingFactor]-self.complianceAges[enum],(np.array([self.modifiedDPLfunction((t,self.complianceAges[enum]),popt[0],popt[1],popt[2],popt[3],popt[4],popt[5]) for t in complianceSeries[0][::plotSlicingFactor]]))*conversionFactor_compliance*(1e6), c=c, marker='x',markersize=mrkSz,linestyle='-', label="DPL-$t_{0}$="+"{:.1f}".format(self.complianceAges[enum])+"d",zorder=99)
                elif typeOfDPL == 'simplified':
                    if axisObjectToPlot is None:
                        plt.plot(complianceSeries[0][::plotSlicingFactor]-self.complianceAges[enum],self.computeSimplifiedPowerLaw(complianceSeries[0][::plotSlicingFactor],self.complianceAges[enum])*(1e6), c=c, marker='x',markersize=mrkSz,linestyle='--', label="DPL-"+"{:.1f}".format(self.complianceAges[enum])+"d",zorder=99)
                    else: 
                        axisObjectToPlot.plot(complianceSeries[0][::plotSlicingFactor]-self.complianceAges[enum],(np.array([self.computeSimplifiedPowerLaw(t,self.complianceAges[enum]) for t in complianceSeries[0][::plotSlicingFactor]]))*(1e6), c=c, marker='x',markersize=mrkSz,linestyle='-', label="DPL-$t_{0}$="+"{:.1f}".format(self.complianceAges[enum])+"d",zorder=99)                
                else:
                    0 #Do nothing
                if plotOriginalSeries is True:
                    plt.plot(np.array(self.complianceSeries[enum][0][::plotSlicingFactor])-self.complianceAges[enum],self.complianceSeries[enum][1][::plotSlicingFactor]*(1e6), c=c, marker='o',markersize=mrkSz,linestyle='-', label="rawData",zorder=99)
                    # Plot the standard variation as a shaded region
                    plt.fill_between(np.array(self.complianceSeries[enum][0][::plotSlicingFactor])-self.complianceAges[enum], 
                                    self.complianceSeries[enum][1][::plotSlicingFactor]*(1e6) - self.complianceVariation[enum][::plotSlicingFactor]*(1e6), 
                                    self.complianceSeries[enum][1][::plotSlicingFactor]*(1e6) + self.complianceVariation[enum][::plotSlicingFactor]*(1e6), 
                                    color=c, alpha=0.15, label=None,zorder=99)
    ...
    def simplifiedDPLfunction(self, T,  ϕ_1, p,q, m, n):
        """
        ...
        return np.log10(ϕ_1)-m*(np.log10((np.array(t_line))))+n*np.log10((np.array(t-t_line)))+np.log10(np.e)*((p/(t_line))**q)
        # return np.log10(ϕ_1)-m*(np.log10((np.array(t_line))))**p+n*np.log10((np.array(t-t_line)))+np.log10((np.array(t-t_line)*np.array(t_line)))*q
        # return np.log10(ϕ_1)+n*np.log10((np.array(t-t_line)))+m*np.log10(np.e)*(p/(t_line))**q
    ...
    def DPLfunction(self, T, E_0, ϕ_1, m, n):
        ...
        t,t_line=T        
        return ((1/(E_0)))+(ϕ_1/E_0)*((np.array(t_line))**(-m))*((np.array(t-t_line))**(n))
        # return (ϕ_1)*((np.array(t-t_line))**(n))

    def modifiedDPLfunction(self, T, E_0, ϕ_1, p, q, m, n):
        """
    ...
    def computeModifiedDoublePowerLaw(self, creepTimesList: list, loadingAge:float):
        ...
        J=np.array([self.modifiedDPLfunction((t,loadingAge),self.modifiedDoublePowerLawModel['E0'],self.modifiedDoublePowerLawModel['ϕ_1'],self.modifiedDoublePowerLawModel['p'],self.modifiedDoublePowerLawModel['q'],self.modifiedDoublePowerLawModel['m'],self.modifiedDoublePowerLawModel['n']) for t in creepTimesList])*self.modifiedDoublePowerLawModel['conversionFactor_compliance']
        
        return J
    
    def computeSimplifiedPowerLaw(self, creepTimesList: list, loadingAge:float):
        """
        A method to compute the values of the fitted DPL stored in self.doublePowerLawModel. It returns a list containing the points of the DPL in each creep time given in creepTimesList, for a given loadingAge.
        ...
        J=np.array([10**self.simplifiedDPLfunction((t,loadingAge),self.simplifiedPowerLawModel['ϕ_1'],self.simplifiedPowerLawModel['p'],self.simplifiedPowerLawModel['q'],self.simplifiedPowerLawModel['m'],self.simplifiedPowerLawModel['n'])*self.simplifiedPowerLawModel['conversionFactor_compliance'] for t in creepTimesList])
    ...
    def fitDirichletSeries(self, fittingData: dict, loadingAgesInterval: list=None, creepTimesInterval: list=None, qμ:float=1, retardationTimesRange:list=None, retardationTimesFactor:int=None, graphVisualization:bool=False, mrkSz:int=5, plotSlicingFactor:int=1, axisObjectToPlot:object=None):
        ...
        elif fittingData['model']=='sDPL':
            t_lines_real=np.geomspace(loadingAgesInterval[0],loadingAgesInterval[1],num=loadingAgesInterval[2])
            t_real=[np.geomspace(creepTimesInterval[0],creepTimesInterval[1],num=creepTimesInterval[2])+t_line for t_line in t_lines_real]
            #J_real=np.array([self.computeDoublePowerLaw(t,t_line) for t_line, t in zip(t_lines_real, t_real)])/self.doublePowerLawModel['conversionFactor_compliance']
            conversionFactor=self.simplifiedPowerLawModel['conversionFactor_compliance']
            #conversionFactor=1
            J_real=np.array([self.computeSimplifiedPowerLaw(t,t_line) for t_line, t in zip(t_lines_real, t_real)])*(1e6)
            #modulusGuess=self.doublePowerLawModel['E0']/conversionFactor
            modulusGuess=1
        else:
        ...
        #Show how good the optimization was
        if graphVisualization is True:
            color = iter(plt.cm.viridis(np.linspace(0, 1, len(t_lines_real))))
            for enum, t_lines in enumerate(t_lines_real):
                c = next(color)
                plt.plot(t_real[enum][::plotSlicingFactor]-t_lines, J_real[enum][::plotSlicingFactor], label="data-"+"{:.1f}".format(t_lines)+" days", c=c, marker='o', linestyle='none')
                J_dirichletSeries = self.dirichletFunction(retardationTimes, qμ, t_lines, t_real[enum], modulusList[enum])
                if axisObjectToPlot is None:
                    plt.plot(t_real[enum][::plotSlicingFactor]-t_lines, J_dirichletSeries[::plotSlicingFactor], label="KC-$t_{0}$="+"{:.1f}".format(t_lines)+"d", c=c, marker='v',markersize=mrkSz, linestyle='--')
                else:
                    axisObjectToPlot.plot(t_real[enum][::plotSlicingFactor]-t_lines, J_dirichletSeries[::plotSlicingFactor], label="KC-$t_{0}$="+"{:.1f}".format(t_lines)+"d", c=c, marker='v',markersize=mrkSz, linestyle='--')
    ...
    def getInterpolatedAgeingModulus(self,t_query):
        #Generated from ChatGPT! Take lightly
        t_lines = np.array(self.dirichletSeriesModel['t_line'])
        aging_modulus = np.array(self.dirichletSeriesModel['agingModulus'])  # Assuming a 2D array (N x len(t_lines))
        
        # Check if t_query is exactly in t_lines
        if t_query in t_lines:
            return aging_modulus[np.where(t_lines == t_query)[0][0]]
        
        # If not, perform linear interpolation
        if t_query < t_lines.min() or t_query > t_lines.max():
            raise ValueError("t_query is out of the available range of t_lines.")
        
        # Find indices of the closest two points
        idx_below = np.max(np.where(t_lines < t_query))
        idx_above = np.min(np.where(t_lines > t_query))
        
        # Perform linear interpolation for each aging modulus component
        t1, t2 = t_lines[idx_below], t_lines[idx_above]
        m1, m2 = aging_modulus[idx_below], aging_modulus[idx_above]
        
        interpolated_value = m1 + (m2 - m1) * (t_query - t1) / (t2 - t1)
        return interpolated_value

    def compareDirichletRealData(self, model: str="simplified", loadingAgesInterval: list=None, creepTimesInterval: list=None, mrkSz:int=5, plotSlicingFactor:int=1, axisObjectToPlot:object=None, rawData: list=None):
        t_lines_real=loadingAgesInterval

        retardationTimes=self.dirichletSeriesModel['retardationTimes']
        qμ=self.dirichletSeriesModel['qu']
        modulusList=self.dirichletSeriesModel['agingModulus']
         
        #Show how good the optimization was
        color = iter(plt.cm.viridis(np.linspace(0, 1, len(t_lines_real))))
        for enum, t_lines in enumerate(t_lines_real):
            c = next(color)
            t_real=creepTimesInterval+t_lines
            ageingModulus=self.getInterpolatedAgeingModulus(t_lines)

            if model=="simplified":
                J_curveFit=self.computeSimplifiedPowerLaw(t_real,t_lines)*1e6
                J_dirichletSeries = self.dirichletFunction(retardationTimes, qμ, t_lines, t_real, ageingModulus)
            elif model=="modified":
                J_curveFit=self.computeModifiedDoublePowerLaw(t_real,t_lines)*1e6
                J_dirichletSeries = self.dirichletFunction(retardationTimes, qμ, t_lines, t_real, ageingModulus)*self.modifiedDoublePowerLawModel['conversionFactor_compliance']*1e6
            elif model=="raw":
                J_dirichletSeries = self.dirichletFunction(retardationTimes, qμ, t_lines, t_real, ageingModulus)*self.dirichletSeriesModel['conversionFactor_compliance']*1e6
                time_raw=rawData[0]
                J_raw=rawData[1]*1e6

            if axisObjectToPlot is None:
                #Plot Kelvin-Chain
                plt.plot(t_real[::plotSlicingFactor]-t_lines, J_dirichletSeries[::plotSlicingFactor], label="KC-$t_{0}$="+"{:.1f}".format(t_lines)+"d", c=c, marker='v',markersize=mrkSz, linestyle='--')
                #Plot experimental curve
                if model == "raw":
                    plt.plot(time_raw[::plotSlicingFactor]-t_lines, J_raw[::plotSlicingFactor], label="raw-$t_{0}$="+"{:.1f}".format(t_lines)+"d", c=c, marker='o',markersize=mrkSz, linestyle='--',zorder=99)
                else:
                    plt.plot(t_real[::plotSlicingFactor]-t_lines, J_curveFit[::plotSlicingFactor], label="SC-$t_{0}$="+"{:.1f}".format(t_lines)+"d", c=c, marker='o',markersize=mrkSz, linestyle='--',zorder=99)
            else:
                axisObjectToPlot.plot(t_real[::plotSlicingFactor]-t_lines, J_dirichletSeries[::plotSlicingFactor], label="KC-$t_{0}$="+"{:.1f}".format(t_lines)+"v", c=c, marker='v',markersize=mrkSz, linestyle='--',zorder=99)
        
                if model == "raw":
                    axisObjectToPlot.plot(time_raw[::plotSlicingFactor]-t_lines, J_raw[::plotSlicingFactor], label="raw-$t_{0}$="+"{:.1f}".format(t_lines)+"d", c=c, marker='o',markersize=mrkSz, linestyle='--',zorder=99)
                else:
                    axisObjectToPlot.plot(t_real[::plotSlicingFactor]-t_lines, J_curveFit[::plotSlicingFactor], label="SC-$t_{0}$="+"{:.1f}".format(t_lines)+"d", c=c, marker='o',markersize=mrkSz, linestyle='--',zorder=99)

                ####MAKE THE PLOT OF RAW DATA HERE
                axisObjectToPlot.plot(np.array(self.complianceSeries[enum][0])-self.complianceAges[enum],self.complianceSeries[enum][1]*(1e6), c=c, markersize=mrkSz,linestyle='-', label="raw-$t_{0}$="+"{:.1f}".format(t_lines)+"d",zorder=99)
                    # Plot the standard variation as a shaded region
                axisObjectToPlot.fill_between(np.array(self.complianceSeries[enum][0])-self.complianceAges[enum], 
                                    self.complianceSeries[enum][1]*(1e6) - self.complianceVariation[enum]*(1e6), 
                                    self.complianceSeries[enum][1]*(1e6) + self.complianceVariation[enum]*(1e6), 
                                    color=c, alpha=0.15, label=None,zorder=99)
